...
namespace Multiboot
{
   struct MultibootStructure
   {
      ...
      inline void printField(const char* name, uint32_t value) const
      {
         printf("%s: 0x%x (%u)\n", name, value, value);
      }
      ...
      inline void printField(const char* name, const char* value) const
      {
         printf("%s: %s\n", name, value);
      }

      inline void printField(MemoryMap* mmap) const
      {
         const char* typeName;
         switch (mmap->type)
         {
